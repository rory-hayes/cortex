...
The post-`RFB-089` release wiring pass pushed `main`, launched the Vercel production app at `https://cortex-two-mu.vercel.app`, and smoke-tested `rory-hayes/payslip-peeks-and-probes.git` through metadata-only GitHub inventory, repo-readiness scan modules, Playwright public/protected route checks, and a local runner dry run. `docs/MVP_RELEASE_NOTES.md` now includes a runtime setup status section for production credentials and Supabase CLI boundaries, and `docs/PRODUCTION_RUNTIME_SETUP.md` documents the required production environment variables without secret values. A 2026-06-03 signup repair switched the active auth shell from Clerk to Auth0, added the Auth0 client/proxy/session wiring, kept `/sign-up` as a compatibility route, and verified the configured signup path redirects to Auth0 with `screen_hint=signup`; a June 4, 2026 redirect repair now sends sign-in and sign-up transactions back to `/dashboard` after authentication and updates production smoke coverage for that return target. A June 4 replacement-project recovery linked the new Supabase project, applied canonical migrations `0000` through `0022`, verified direct database connectivity through the app's server-side Postgres client, and configured `DATABASE_URL` in Vercel production. The runner dry-run probe surfaced and fixed a capability-detection mutation path where Corepack/Yarn could edit a target repo `package.json`; capability probes now run from a neutral probe directory. Full production GitHub-connect/setup-PR E2E remains blocked until GitHub App runtime credentials are configured.

A follow-up verification pass added `pnpm release-readiness:check`, which composes sanitized local runtime env checks, Vercel production env-name checks, GitHub App runtime identity, deployed route smoke, local Supabase link, Supabase migration history, direct Supabase database connectivity, and Supabase endpoint checks into one release gate. `pnpm github-app:check` is also available as a focused GitHub App credential verifier. Its text and JSON output stay status-only and avoid secret values, Supabase refs, database URLs, API keys, private keys, JWTs, webhook secrets, response bodies, local paths, query output, and raw database client errors.

The release operator path now also includes `pnpm vercel-production-env:apply --dry-run`, which validates remaining required production values from the current shell and previews missing Vercel production variables before applying them without printing values. `pnpm vercel-production-env:apply --key DATABASE_URL` supports replacement-project recovery when only one required secret is ready to apply.

A June 4 operational-control-room polish pass added a product-design spec under `docs/superpowers/specs/`, strengthened the public landing page with a code-native Cortex control-loop preview, grouped the authenticated navigation by command/readiness/execution/governance, sharpened dashboard overview and repo-readiness onboarding hierarchy, improved shared empty-state and outline-button treatment, and added explicit icon metadata. The pass preserves Auth0 sign-in/sign-up dashboard return targets and the hosted metadata-only trust boundary.

A June 4 continuation audit reconfirmed the deployed public/signup/protected route smoke, production Auth0 sign-in/sign-up redirects to the configured tenant with dashboard return targets, Vercel production Auth0 plus `DATABASE_URL` configuration, and Supabase link/migration/endpoint readiness. Full production GitHub-connect/setup-PR E2E remains blocked only by the missing required GitHub App runtime credentials.

A June 4 no-GitHub-App E2E pass added a public GitHub URL scan path for public repositories. The dashboard onboarding can now register a public GitHub repo from URL, store only safe repository metadata under a synthetic public installation marker, and queue the existing metadata-only repo scan flow without GitHub App credentials. Focused server/UI coverage proves the public path rejects unsafe hidden fields and still hands off to scan modules that create findings, recommendations, and draft Cortex Tasks; a live metadata-only inventory probe against `rory-hayes/payslip-peeks-and-probes.git` found the expected readiness gaps for agent instructions, backlog quality, validation policy, CI, product docs, policy coverage, and mixed package locks. The change was pushed to `rory-hayes/cortex.git`, Vercel auto-deployed it to production, and sanitized production smoke returned ready.
