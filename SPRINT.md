...
The release operator path now also includes `pnpm vercel-production-env:apply --dry-run`, which validates remaining required production values from the current shell and previews missing Vercel production variables before applying them without printing values.
