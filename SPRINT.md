...
A follow-up verification pass added `pnpm release-readiness:check`, which composes sanitized local runtime env checks, Vercel production env-name checks, deployed route smoke, local Supabase link, Supabase migration history, direct Supabase database connectivity, and Supabase endpoint checks into one release gate. Its text and JSON output stay status-only and avoid secret values, Supabase refs, database URLs, API keys, private keys, response bodies, local paths, query output, and raw database client errors.
