...
const post = async (body: unknown) => {
  ...
  return POST(
    new Request("https://control-plane.test/api/runner/runs/validation-result", {
...
beforeEach(() => {
  ...
  mocks.createRunArtifactSubmissionService.mockReturnValue({
    submitValidationResult: mocks.submitValidationResult,
  });
  mocks.submitValidationResult.mockResolvedValue(validValidationResult());
});

describe("POST /api/runner/runs/validation-result", () => {
  ...
  test("invalid JSON returns safe 400 invalid_request", async () => {
    ...
    expect(mocks.getDatabase).not.toHaveBeenCalled();
    expect(mocks.submitValidationResult).not.toHaveBeenCalled();
  });
  ...
  test("schema-invalid request returns safe 400 without Zod internals", async () => {
    ...
    expect(responseText).not.toMatch(/Zod|runId|run_2|result/i);
    expect(mocks.getDatabase).not.toHaveBeenCalled();
    expect(mocks.submitValidationResult).not.toHaveBeenCalled();
  });
  ...
  test("unsafe schema-valid validation summary returns safe 400 before database or service work", async () => {
    const response = await post(
      validRequestBody({
        result: validValidationResult({
    ...
    expect(mocks.createRunArtifactSubmissionService).not.toHaveBeenCalled();
    expect(mocks.submitValidationResult).not.toHaveBeenCalled();
    expect(responseText).not.toMatch(/raw output|unsafe command result|request body|Zod/i);
  });

  test("validation result without redaction applied returns safe 400 before database or service work", async () => {
    const response = await post(
      validRequestBody({
        result: validValidationResult({
    ...
    expect(mocks.createRunArtifactSubmissionService).not.toHaveBeenCalled();
    expect(mocks.submitValidationResult).not.toHaveBeenCalled();
    expect(responseText).not.toMatch(/redactionApplied|request body|Zod/i);
  });
  ...
  test("valid request creates DB-backed service and returns ok data", async () => {
    ...
    expect(mocks.submitValidationResult).toHaveBeenCalledWith({
      context: {
        runnerId: "runner_1",
        workspaceId: "workspace_1",
      },
  ...
  test("service-level artifact errors map to generic invalid_request", async () => {
    mocks.submitValidationResult.mockRejectedValue(new mocks.RunArtifactSubmissionError());
    const response = await post(validRequestBody());
    const responseText = await response.text();
