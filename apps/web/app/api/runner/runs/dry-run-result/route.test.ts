...
const post = async (body: unknown) => {
  ...
  return POST(
    new Request("https://control-plane.test/api/runner/runs/dry-run-result", {
...
beforeEach(() => {
  ...
  mocks.createRunArtifactSubmissionService.mockReturnValue({
    submitDryRunResult: mocks.submitDryRunResult,
  });
  mocks.submitDryRunResult.mockResolvedValue(validDryRunResult());
});

describe("POST /api/runner/runs/dry-run-result", () => {
  ...
  test("invalid JSON returns safe 400 invalid_request", async () => {
    ...
    expect(mocks.getDatabase).not.toHaveBeenCalled();
    expect(mocks.submitDryRunResult).not.toHaveBeenCalled();
  });
  ...
  test("unsafe schema-valid dry-run text returns safe 400 before database or service work", async () => {
    const response = await post(
      validRequestBody({
        result: validDryRunResult({
          checks: [
  ...
  test("valid request creates DB-backed service and returns ok data", async () => {
    ...
    expect(mocks.submitDryRunResult).toHaveBeenCalledWith({
      context: {
        runnerId: "runner_1",
        workspaceId: "workspace_1",
      },
  ...
  test("service-level artifact errors map to generic invalid_request", async () => {
    mocks.submitDryRunResult.mockRejectedValue(new mocks.RunArtifactSubmissionError());
    const response = await post(validRequestBody());
    const responseText = await response.text();
