...
Additional 2026-06-04 Runtime Recheck: Added `pnpm vercel-production-env:apply`, which validates required production values from the current shell and applies only missing required Vercel production variables without printing values. Passed focused release-readiness/doc guards with `pnpm test -- scripts/apply-vercel-production-env.test.ts scripts/check-release-readiness.test.ts scripts/check-vercel-production-env.test.ts scripts/codex-runner/mvp-release-notes.test.ts scripts/codex-runner/production-runtime-doc.test.ts`; `pnpm vercel-production-env:apply --dry-run --json` returned sanitized blocked output naming only the missing current-shell `DATABASE_URL` and GitHub App keys; `git diff --check`; and live `pnpm release-readiness:check --app-url <deployed-app-url> --supabase-url <supabase-project-url> --json` returned sanitized blocked output with local shell runtime variables absent, Vercel Auth0 variables configured, Vercel `DATABASE_URL` and GitHub App variables absent, production route smoke ready, Supabase link ready, Supabase migration history ready, direct database verification blocked until `DATABASE_URL` is loaded, and Supabase endpoint smoke ready.
Additional 2026-06-04 Replacement Supabase Recheck: Linked the replacement Supabase project locally, applied canonical package migrations `0000` through `0022` through a temporary Supabase CLI migration layout, and verified linked remote history with `pnpm supabase-migrations:check --json` reporting local and remote counts at 23/latest `0022`. Added a sanitized Node Postgres fallback to `pnpm supabase-db:check` for machines without `psql`, added `pnpm vercel-production-env:apply --key <required-key>` for single-key production env recovery, applied only `DATABASE_URL` to Vercel production, and added `docs/PRODUCTION_E2E_REPORT.md` for the no-secret recovery report. Validation passed: `pnpm supabase-link:check --json`; `pnpm supabase-migrations:check --json`; `pnpm supabase-smoke:check --url <supabase-project-url> --json`; `pnpm supabase-db:check --json` with the replacement `DATABASE_URL` loaded only in-process; `pnpm vercel-production-env:apply --key DATABASE_URL --dry-run --json`; `pnpm vercel-production-env:apply --key DATABASE_URL --json`; `pnpm vercel-production-env:check --json` confirmed Vercel production has Auth0 plus `DATABASE_URL` configured and only GitHub App required keys missing; `pnpm release-readiness:check --app-url <deployed-app-url> --supabase-url <supabase-project-url> --json` reported production smoke, Supabase link, Supabase migrations, Supabase database, and Supabase endpoint smoke ready while blocking only required GitHub App runtime keys plus optional Linear warnings; `vercel --prod --yes` deployed and aliased production; post-deploy `pnpm production-smoke:check --url <deployed-app-url> --json` passed; in-app browser smoke verified the production homepage, signup links, and Auth0 signup redirect; `pnpm run typecheck`; `pnpm run lint`; `pnpm run format:check`; `pnpm run build` passed with expected local-shell Auth0 warnings; and `pnpm test` (264 files, 3186 tests).
Additional 2026-06-04 GitHub App Runtime Verifier: Added `pnpm github-app:check` to validate required GitHub App runtime values, local App JWT signing, and live `/app` identity matching without printing private keys, JWTs, webhook secrets, raw GitHub errors, or response bodies. Added a `github_app_runtime` section to `pnpm release-readiness:check` so the remaining production blocker is visible in the combined safe gate, and updated the production runtime docs, MVP release notes, recovery report, README, and SPRINT status. Validation passed: focused GitHub/release/runtime guard suite (`pnpm test -- scripts/check-github-app-runtime.test.ts scripts/check-release-readiness.test.ts scripts/check-production-runtime.test.ts scripts/check-vercel-production-env.test.ts scripts/check-supabase-db.test.ts scripts/check-supabase-migrations.test.ts scripts/check-supabase-link.test.ts scripts/check-supabase-smoke.test.ts scripts/check-production-smoke.test.ts scripts/codex-runner/production-runtime-doc.test.ts scripts/codex-runner/mvp-release-notes.test.ts`, 11 files, 52 tests); `pnpm github-app:check --json` returned sanitized blocked output naming only missing GitHub App keys in the current shell; `pnpm release-readiness:check --app-url <deployed-app-url> --json` returned sanitized blocked output with Vercel Auth0 plus `DATABASE_URL` configured, Vercel GitHub App keys missing, production smoke ready, Supabase link ready, Supabase migrations ready, and current-shell GitHub App/database/Supabase URL values absent; `pnpm run typecheck`; `pnpm run lint`; `pnpm run format:check`; `git diff --check`; `pnpm test` (265 files, 3192 tests); and `pnpm run build` passed with expected local-shell Auth0 warnings.
Additional 2026-06-04 Operational Control Room Polish: Added `docs/superpowers/specs/2026-06-04-operational-control-room-polish-design.md` and implemented the approved product-design direction across the public landing page, authenticated navigation shell, dashboard overview, repo-readiness onboarding, shared empty states, outline-button treatment, and app icon metadata. The pass preserves Auth0 dashboard return targets, the scan-before-runner onboarding order, source-boundary copy, and metadata-only hosted payload language while improving first-screen product signal, status hierarchy, action rows, and mobile public-page structure. Validation passed: focused UI/source suite (`pnpm test -- apps/web/app/page.test.ts apps/web/app/navigation-shell.test.ts apps/web/app/overview-page-ui.test.tsx apps/web/app/task-184-browser-happy-path-smoke.test.tsx apps/web/app/ui-baseline.test.ts`, 5 files, 25 tests); local browser smoke of `http://localhost:3001` at desktop and mobile widths verified the public entry, code-native control-loop preview, sign-in/sign-up links, and clean dev console except normal React/Next dev messages; local `/dashboard` returned the expected missing-Auth0 503 in the unconfigured shell; `pnpm run typecheck`; `pnpm run lint`; `pnpm run format:check`; `pnpm test` (265 files, 3192 tests); and `pnpm run build` passed with expected local-shell Auth0 warnings.
Additional 2026-06-04 Continuation Audit: Re-ran sanitized production readiness checks and in-app browser smoke against the deployed app. `pnpm vercel-production-env:check --json` confirmed Vercel production has `WEB_BASE_URL`, `APP_BASE_URL`, all required Auth0 variables, and `DATABASE_URL` configured, with only `GITHUB_APP_ID`, `GITHUB_APP_PRIVATE_KEY`, and `GITHUB_WEBHOOK_SECRET` missing from the required set. `pnpm production-smoke:check --url <deployed-app-url> --json` returned ready for the public route, sign-up Auth0 redirect, and protected dashboard redirect. The in-app browser confirmed the production landing page renders, sign-in links use `returnTo=/dashboard`, sign-up links use `screen_hint=signup&returnTo=/dashboard`, signup reaches the configured Auth0 tenant's `/u/signup`, and login reaches `/u/login`. `pnpm release-readiness:check --app-url <deployed-app-url> --supabase-url <project-url> --json` still reported Supabase link, Supabase migration history, Supabase endpoint smoke, and production route smoke as ready while blocking on missing local-shell runtime values and the Vercel GitHub App credential set. A temporary Vercel env pull produced no local key material, so the direct database check was not rerun from pulled production values in this continuation; the previous credential-backed direct database check remains the database proof, and the fresh Vercel env-name check confirms `DATABASE_URL` is still configured in production.
Additional 2026-06-04 No-GitHub-App Public Repo E2E: Added a public GitHub URL scan path so users can start repo readiness for public repositories without installing a GitHub App. The implementation adds a bounded public GitHub REST request adapter, public repository URL parsing/registration, synthetic public installation IDs, public inventory reads in the existing GitHub inventory facade, a `triggerPublicRepoScanAction`, and a dashboard onboarding form that submits only canonical `workspaceId`, `repositoryUrl`, and optional `productGoal`. The path preserves the hosted metadata-only boundary and queues the existing scan flow that turns findings into task recommendations and draft Cortex Tasks. Validation passed: focused red/green public GitHub scan suite; `pnpm --filter @control-plane/web test -- src/github/public-request.test.ts src/github/public-repositories.test.ts src/repo-readiness/github-inventory.test.ts src/server/action-factories.test.ts app/overview-page-ui.test.tsx src/dashboard/overview.test.ts app/task-184-browser-happy-path-smoke.test.tsx src/repo-readiness/onboarding-e2e.test.tsx`; live metadata-only inventory probe against `rory-hayes/payslip-peeks-and-probes.git` showing missing `AGENTS.md`, missing backlog quality, missing validation policy/commands, missing CI, missing product docs, missing policy file, and mixed package locks; local `/sign-up` browser smoke showing the expected Auth0 setup fallback when local runtime keys are absent; `pnpm typecheck`; `pnpm lint`; `pnpm format:check`; `pnpm --filter @control-plane/web build` with expected local-shell Auth0 warnings; push to `origin/main`; content-equivalent push to `rory-hayes/cortex.git` `main`; Vercel production auto-deploy ready; and `pnpm production-smoke:check --url <deployed-app-url> --json` ready for public, signup, and protected-route checks.
Follow-up Risk: Post-MVP deferred tasks begin at `RFB-090` and should not start without an explicit scope change. Supabase database setup is no longer the production blocker. The public read-only scan path does not require GitHub App credentials, but production setup-PR writes and GitHub App E2E remain blocked until `GITHUB_APP_ID`, `GITHUB_APP_PRIVATE_KEY`, and `GITHUB_WEBHOOK_SECRET` are configured.
Next Recommended Task: Sign into production, run the public GitHub URL scan for `rory-hayes/payslip-peeks-and-probes.git`, and approve the generated setup recommendations into draft Cortex Tasks; configure GitHub App runtime credentials only when setup PR writes need production verification.
