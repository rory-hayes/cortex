...
Follow-up Risk: Post-MVP deferred tasks begin at `RFB-090` and should not start without an explicit scope change. Supabase database setup is no longer the production blocker. Full production GitHub-connect/setup-PR E2E remains blocked until `GITHUB_APP_ID`, `GITHUB_APP_PRIVATE_KEY`, and `GITHUB_WEBHOOK_SECRET` are configured.
Next Recommended Task: Configure GitHub App runtime credentials, run `pnpm github-app:check`, redeploy production, then rerun production GitHub-connect/setup-PR E2E.
